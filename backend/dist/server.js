...
const loadWalletsFromFile = () => {
    try {
        const walletsPath = path_1.default.join(__dirname, '../../data/wallets.txt');
...
app.post('/api/aggregated', (req, res) => {
    try {
        ...
        if (selectedWallets && Array.isArray(selectedWallets) && selectedWallets.length > 0) {
            ...
            filteredWallets = walletsData.filter(wallet => selectedAddresses.includes(wallet.address.toLowerCase()));
...
app.post('/api/export/csv', async (req, res) => {
    try {
        ...
        if (selectedWallets && Array.isArray(selectedWallets) && selectedWallets.length > 0) {
            const selectedAddresses = selectedWallets.map(addr => addr.trim().toLowerCase());
            dataToExport = walletsData.filter(wallet => selectedAddresses.includes(wallet.address.toLowerCase()));
        ...
        if (dataToExport.length > batchSize) {
            ...
            for (let i = 0; i < dataToExport.length; i += batchSize) {
                ...
                const batchCsv = batch.map(wallet => `"${wallet.address}",${wallet.totalValue},"${wallet.chains?.length || 0}","${wallet.tokens?.length || 0}","${wallet.protocols?.length || 0}","${wallet.lastUpdated}"`).join('\n') + '\n';
                res.write(batchCsv);
...
app.post('/api/logs', (req, res) => {
    try {
        ...
        if (walletAddress) {
            filteredLogs = logs.filter(log => log.message && log.message.includes(walletAddress));
        }
        res.json({
            logs: filteredLogs,
            total: logs.length,
...
app.get('/api/protocols/:id/wallets', (req, res) => {
    try {
        ...
        // Filter by selected wallets if provided
        if (selectedWallets && Array.isArray(JSON.parse(selectedWallets))) {
            const selectedAddresses = JSON.parse(selectedWallets).map((addr) => addr.trim().toLowerCase());
            walletsToAnalyze = walletsData.filter(wallet => selectedAddresses.includes(wallet.address.toLowerCase()));
        }
        const protocolWallets = dataProcessor.getProtocolWallets(walletsToAnalyze, protocolId, chain);
        // Пагинация для больших объемов данных
        const offsetNum = parseInt(offset || '0');
        const limitNum = parseInt(limit || '100'); // По умолчанию 100 кошельков
...
app.get('/api/tokens/:symbol/wallets', (req, res) => {
    try {
        ...
        const tokenWallets = dataProcessor.getTokenWallets(walletsToAnalyze, tokenSymbol, chain);
        // Пагинация для больших объемов данных
        const offsetNum = parseInt(offset || '0');
        const limitNum = parseInt(limit || '100'); // По умолчанию 100 кошельков
...
app.post('/api/wallets/refresh', async (req, res) => {
    try {
        ...
        setImmediate(async () => {
            try {
                isProcessing = true;
                logo_1.Logo.showProcessingStart(wallets.length);
...
const processWallets = async (addresses, forceUpdate = false) => {
    ...
    const progressBar = new progressBar_1.ProgressBar(addressesToProcess.length);
    ...
    if (!forceUpdate) {
        ...
        const walletsNotBeingUpdated = storedWallets.filter(wallet => !addressesToProcessSet.has(normalizeAddress(wallet.address)));
...
app.get('/api/security/status', (req, res) => {
    try {
        const config = security_1.securityManager.getConfig();
        const isActive = security_1.securityManager.isActive();
        res.json({
            isActive,
            config,
...
app.post('/api/security/toggle', (req, res) => {
    try {
        const { enabled } = req.body;
        if (enabled && !security_1.securityManager.isActive()) {
            security_1.securityManager.initialize();
        }
        else if (!enabled && security_1.securityManager.isActive()) {
            security_1.securityManager.disable();
        }
        const isActive = security_1.securityManager.isActive();
...
app.put('/api/security/config', (req, res) => {
    try {
        ...
        config_1.configManager.updateSecurityConfig(newConfig);
        // Применяем изменения в менеджере безопасности
        security_1.securityManager.updateConfig(config_1.configManager.getSecurityConfig());
        const config = security_1.securityManager.getConfig();
