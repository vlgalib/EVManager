...
class ProxyService {
    constructor() {
        ...
        this.CIRCUIT_BREAKER_THRESHOLD = 5; // Количество ошибок для размыкания
        this.CIRCUIT_BREAKER_TIMEOUT = 300000; // 5 минут до попытки восстановления
        this.lastCheckDate = '';
        this.loadProxies = () => {
            try {
                const proxyFilePath = path_1.default.join(process.cwd(), '..', 'data', 'proxy.txt');
        ...
        this.GEOIP_CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // 7 days
        this.lastGeoIPRequest = 0;
        this.GEOIP_REQUEST_INTERVAL = 5000; // 5 seconds between requests
        this.pendingGeoIPRequests = new Set(); // Track pending requests
        this.isCheckingProxies = false; // Track proxy checking state
        // Get country from external GeoIP services with fallback and caching
        this.getCountryFromGeoIP = async (ip) => {
                ...
                const services = [
                    {
                        ...
                        parseResponse: (data) => data?.status === 'success' ? data.country : null
                    },
                    {
                        name: 'ipapi.co',
                    ...
                    },
                    {
                        ...
                        parseResponse: (data) => data?.country || null
                    }
                ];
        ...
        this.checkAllProxies = async () => {
            ...
            try {
                ...
                const sortedProxies = authProxies.sort((a, b) => {
                    ...
                    const getStatus = (isWorking, isFailed) => {
                        if (!isWorking && !isFailed)
                            return 0; // Непроверенный
                        if (isFailed && !isWorking)
                            return 1; // Неработающий
                        if (isWorking && !isFailed)
                            return 2; // Работающий
        ...
        this.loadGeoIPCache = () => {
            try {
                const geoIPCacheFilePath = path_1.default.join(process.cwd(), '..', 'data', 'geoip_cache.json');
                if (!fs_1.default.existsSync(geoIPCacheFilePath)) {
                    this.logger.debug('GeoIP cache file not found, starting with empty cache');
                    return;
                }
                const cacheContent = fs_1.default.readFileSync(geoIPCacheFilePath, 'utf-8');
        ...
        this.saveGeoIPCache = () => {
            try {
                ...
                fs_1.default.writeFileSync(geoIPCacheFilePath, JSON.stringify(cacheData, null, 2));
