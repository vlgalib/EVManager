...
class DeBankService {
    constructor() {
        ...
        this.cache = new Map();
        this.cacheTimeout = 45 * 60 * 1000; // 45 минут кэш для снижения нагрузки
        this.getWalletData = async (walletAddress, forceUpdate = false, walletId) => {
        ...
        this.scrapeWalletData = async (walletAddress, proxy, walletId) => {
        ...
        this.processWalletData = (walletAddress, networkData, walletId) => {
                                ...
                                protocol.portfolio_item_list.forEach((item, itemIndex) => {
                                    ...
                                    if (item.detail && item.detail.description) {
                                        ...
                                        if (desc.other_token_list) {
                                            desc.other_token_list.forEach((token) => {
                                                if (token.amount > 0) {
                                                    protocolTokens.push({
                                                        symbol: token.symbol || 'Unknown',
                                                        name: token.name || token.symbol || 'Unknown Token',
